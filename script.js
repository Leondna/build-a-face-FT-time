...
function updateMovement(value) {
  face.classList.remove("still", "wiggle", "bounce");

  if (value < 20) {
    face.classList.add("still");
  } else if (value < 65) {
...
function getMoodLabel(state) {
  const { mouth, eyes, brows, blush, look, move } = state;

  if (mouth > 55 && blush > 50 && move > 60) return "Delighted";
  ...
  if (eyes < 30 && move < 20) return "Sleepy";
  if (brows < -20 && mouth >= 0 && Math.abs(look) > 40) return "Nervous";
  if (brows > 20 && mouth >= 0) return "Interested";
  if (Math.abs(look) > 65 && move < 40) return "Thinking";
  if (move > 70 && mouth >= 0) return "Excited";
...
function randomFace() {
  ...
  moveLevel.value = randomInRange(0, 100);
  updateFace();
}
...
function getAnimationOffsets(move, elapsedMs) {
  if (move < 20) {
  ...
  if (move < 65) {
    return {
      dx: 0,
      dy: Math.sin(t * Math.PI * 2) * 2,
      rotation: Math.sin(t * Math.PI * 2) * 0.04
    };
  }

  return {
    dx: 0,
    dy: Math.sin(t * Math.PI * 2 / 0.7) * -10,
    rotation: 0
  };
}

function drawFaceFrame(ctx, state, elapsedMs) {
  ...
  const offsets = getAnimationOffsets(state.move, elapsedMs);
  const cx = w / 2 + offsets.dx;
  const cy = h / 2 - 20 + offsets.dy;
  const r = 180;
  ...
  ctx.beginPath();
  ctx.arc(cx, cy, r - 5, 0.15 * Math.PI, 0.85 * Math.PI, false);
  ...
  ctx.fillText(getMoodLabel(state), w / 2, h - 34);
}
...
function renderLoop(now) {
  animationStart = animationStart || now;
  const state = getState();
  drawFaceFrame(exportCanvas.getContext("2d"), state, now - animationStart);
...
async function downloadAsVideo() {
  ...
  if (!window.MediaRecorder) {
    videoNote.textContent = "This browser does not support video recording from canvas.";
    return;
  }

  isRecording = true;
  downloadVideoBtn.disabled = true;
  videoNote.textContent = "Recording 3-second video...";
  ...
  try {
    ...
    videoNote.textContent = "Video export could not start in this browser.";
  ...
  recorder.onstop = () => {
    ...
    link.href = url;
    link.download = `emotion-face-${stamp}.webm`;
    link.click();

    setTimeout(() => URL.revokeObjectURL(url), 1500);
    videoNote.textContent = "Video downloaded.";
    downloadVideoBtn.disabled = false;
    isRecording = false;
  };
...
[
  ...
  slider.addEventListener("input", updateFace);
});
